...
class MarkdownFormatterEngine {
    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith('.json')) {
          ...
          if (data.type === 'preset') this.presets.set(data.id, data);
        }
      }
    } catch (e) {}
  }
  ...
  async formatMarkdown(content, presetId = 'standard') {
    const preset = this.presets.get(presetId) || this.presets.get('standard');
    ...
    if (rules.consistentEmphasis) {
      ...
      formatted = formatted.replace(/_([^_]+)_/g, '__$1__');
    }
