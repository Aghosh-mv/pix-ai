...
class MarkdownEngine {
  constructor(config, logger) {
    ...
    this.markdownDir = path.join(os.homedir(), '.pix/markdown');
  }

  async initialize() {
    this.logger.info('Initializing Markdown Engine...');
    await fs.ensureDir(this.markdownDir);
    await this.loadDocuments();
    this.loadDefaultFolders();
    this.loadDefaultTemplates();
    this.logger.info('Markdown Engine initialized');
  }
    ...
    try {
      ...
      for (const file of files) {
        if (file.endsWith('.json')) {
          const data = await fs.readJson(path.join(this.markdownDir, file));
